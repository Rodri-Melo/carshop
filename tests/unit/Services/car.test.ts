...
describe('Testes para a rota Cars', function () {
  ...
  it('Deveria ser possivel cadastrar um carro com sucesso', async function () {
  ...
  it('Deve ser possivel retornar todos os carros com sucesso', async function () {
    sinon.stub(Model, 'find').resolves(allCars);
    const result = await service.getAll();
    expect(result).to.be.deep.equal(allCars);
  });
  it('Dever ser possivel buscar um carro pelo id com sucesso', async function () {
    sinon.stub(Model, 'findById').resolves(carOutput);
    
    const result = await service.getByValue('6348513f34c397abcad040b2');
    expect(result).to.be.deep.equal(carOutput);
  });
  it('Deve ser possivel atualzar um carro com sucesso', async function () {
